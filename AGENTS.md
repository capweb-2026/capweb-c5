...
Cavalier est un chatbot en JavaScript natif, sans framework, spécialisé dans l'apprentissage des échecs pour débutants. Il répond avec un cerveau à règles et, à partir de mercredi, avec une IA appelée par le serveur.

Fichiers principaux :

- `public/js/brain.js` : fonctions pures `validateMessage` et `replyTo`, aucun accès à la page ;
- `public/js/persona.js` : identité de Cavalier et fonction pure `validatePersona`, aucun accès à la page ;
...
- Ne jamais modifier `SPEC.md` ni `AGENTS.md` pendant l'implémentation de l'identité.
- Ne jamais remplacer l'identité Cavalier, son emoji, son accueil ou ses trois suggestions sans modifier `SPEC.md` au préalable.
